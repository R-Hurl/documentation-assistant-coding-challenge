...
- **2️⃣ L2**: is an _optional_ distributed cache (any implementation of `IDistributedCache` will work) and serves the purpose of **easing a cold start** or **sharing cached data** with other nodes/pods. It is treated differently than L1, meaning any potential error happening on this level (remember the [fallacies of distributed computing](https://en.wikipedia.org/wiki/Fallacies_of_distributed_computing) ?) can be automatically handled by FusionCache to not impact the overall application, all while (optionally) logging any detail of it for further investigation
